...
def _source_definition_v11_json() -> str:
    ...
    for key, value in predecessor.items():
        if key == "schema_version":
            value = "1.1"
        if key == "source_definition_version":
            successor[key] = "1.1"
            successor["predecessor_source_definition_id"] = predecessor[
                "source_definition_id"
            ]
            successor["predecessor_source_definition_version"] = "1.0"
            successor["predecessor_source_definition_sha256"] = (
                SOURCE_DEFINITION_SHA256
            )
            continue
        if key == "xml":
            xml: dict[str, object] = {}
            for xml_key, xml_value in value.items():
                if xml_key == "permitted_plist_types":
                    xml["permitted_character_references"] = ["&#38;"]
                xml[xml_key] = xml_value
            value = xml
        if key == "track_required_fields":
            value = [
                item
                for item in value
                if not item.startswith(("Genre:", "Sort Artist:"))
            ]
        if key == "track_optional_fields":
            value = [
                "Sort Album:NONBLANK_STRING",
                "Sort Name:NONBLANK_STRING",
                "Genre:NONBLANK_STRING_EXACT_SOURCE_VALUE_IF_PRESENT",
                "Sort Artist:NONBLANK_STRING_EXACT_SOURCE_VALUE_IF_PRESENT",
                "Album Artist:NONBLANK_STRING_CONFORMANCE_ONLY_IF_PRESENT",
                "Composer:NONBLANK_STRING_CONFORMANCE_ONLY_IF_PRESENT",
                "Artwork Count:CANONICAL_INTEGER_EXACT_1_OBSERVED_CONFORMANCE_ONLY_IF_PRESENT",
            ]
        successor[key] = value
    return json.dumps(successor, ensure_ascii=False, separators=(",", ":"))


SOURCE_DEFINITION_V11_JSON = _source_definition_v11_json()
...
def _acquisition_authority_v11_json() -> str:
    predecessor = json.loads(
    ...
    replacements = {
        ...
        "intake_request_schema": "PennyLocalITunesXMLIntakeRequest/1.1",
        "file_selection_evidence_schema": "PennyLocalITunesXMLFileSelectionEvidence/1.1",
        "wrapper_schema": "PennyLocalITunesXMLAcquisitionAuthorityArtifact/1.1",
        "source_definition": (
            "pne.source-definition.itunes-windows-xml-single-playlist/1.1"
        ),
        "capability_declaration": (
            "pne.source-capability.itunes-windows-xml-single-playlist/1.1"
        ),
        "producer": "pne.producer.itunes-windows-xml-acquisition-authority/1.1",
        "verifier": "pne.verifier.itunes-windows-xml-acquisition-authority/1.1",
    }
    predecessor.update(replacements)
    return json.dumps(predecessor, separators=(",", ":"))


ACQUISITION_AUTHORITY_V11_JSON = _acquisition_authority_v11_json()
ACQUISITION_AUTHORITY_V11_SHA256 = (
    "6a07222c5c65c68e72e29b3e306459f1c6b84f7557aa106ba4c6906171871170"
)

SOURCE_TYPE = "pne.source.itunes-windows-xml-single-playlist"
ADAPTER_ID = "pne.adapter.itunes-windows-xml-single-playlist"
ADAPTER_VERSION = "1.0"
ADAPTER_VERSION_V11 = "1.1"
...
CAPABILITY_DECLARATION_V11 = SourceCapabilityDeclaration(
    declaration_id="pne.source-capability.itunes-windows-xml-single-playlist",
    declaration_version="1.1",
    adapter_id=ADAPTER_ID,
    adapter_version=ADAPTER_VERSION_V11,
    capabilities=CAPABILITY_DECLARATION.capabilities,
)
CAPABILITY_DECLARATION_V11_JSON = (
    '{"schema_version":"1.0","declaration_id":"pne.source-capability.itunes-windows-xml-single-playlist","declaration_version":"1.1","adapter_id":"pne.adapter.itunes-windows-xml-single-playlist","adapter_version":"1.1","capabilities":[{"field":"displayed_explicit","authoritative":false},{"field":"release_version_identity","authoritative":false},{"field":"source_catalog_identity","authoritative":true}]}'
)
...
def verify_frozen_definitions() -> bool:
    values = (
        ...
        (SOURCE_DEFINITION_V11_JSON, SOURCE_DEFINITION_V11_SHA256),
        (MAPPING_DEFINITION_JSON, MAPPING_DEFINITION_SHA256),
        (CAPABILITY_DECLARATION_JSON, CAPABILITY_DECLARATION_SHA256),
        (CAPABILITY_DECLARATION_V11_JSON, CAPABILITY_DECLARATION_V11_SHA256),
        (ACQUISITION_AUTHORITY_JSON, ACQUISITION_AUTHORITY_SHA256),
        (ACQUISITION_AUTHORITY_V11_JSON, ACQUISITION_AUTHORITY_V11_SHA256),
    )
    for content, digest in values:
        ...
        if sha256_bytes(content.encode("utf-8")) != digest:
            return False
    return (
        CAPABILITY_DECLARATION.model_dump(mode="json")
        == json.loads(CAPABILITY_DECLARATION_JSON)
        and CAPABILITY_DECLARATION_V11.model_dump(mode="json")
        == json.loads(CAPABILITY_DECLARATION_V11_JSON)
    )
