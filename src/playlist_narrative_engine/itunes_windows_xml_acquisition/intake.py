...
class ITunesWindowsXMLIntakeAuthorityRepository:
    ...
    def __init__(self) -> None:
        self._requests: dict[
            str, PennyLocalITunesXMLIntakeRequest | PennyLocalITunesXMLIntakeRequestV11
        ] = {}
        self._evidence: dict[
            str,
            PennyLocalITunesXMLFileSelectionEvidence
    ...
    def _record_request(
        self,
        request: PennyLocalITunesXMLIntakeRequest | PennyLocalITunesXMLIntakeRequestV11,
        capability: object,
    ) -> None:
...
class PennyLocalITunesXMLIntakeVerifierV11:
    def __init__(
        self,
        repository: ITunesWindowsXMLIntakeAuthorityRepository,
        principal_verifier: LocalPrincipalAuthorityVerifier,
    ...
    def verify_request(
        self,
        request: PennyLocalITunesXMLIntakeRequestV11,
        *,
        require_current_tip: bool = False,
    ) -> bool:
        if not isinstance(request, PennyLocalITunesXMLIntakeRequestV11):
            return False
        recorded = self._repository._requests.get(request.intake_request_id)
        if recorded != request:
            return False
        try:
            validated = PennyLocalITunesXMLIntakeRequestV11.model_validate(
                request.model_dump(mode="json")
            )
            prefix = self._principal_verifier.verified_lineage_prefix_sha256(
                validated.principal_authority,
    ...
    def verify_evidence(
        ...
        *,
        ...
        if recorded != evidence:
            return False
        return _verify_file_selection_evidence_structure_v11(
            evidence,
            principal_verifier=self._principal_verifier,
            require_current_tip=require_current_tip,
        )
...
class PennyLocalITunesXMLIntakeProducerV11:
    """Sole public production seam for source-profile 1.1 native selection."""

    def __init__(self, principal_verifier: LocalPrincipalAuthorityVerifier) -> None:
        self._principal_verifier = principal_verifier
        self._repository = ITunesWindowsXMLIntakeAuthorityRepository()
        self.verifier = PennyLocalITunesXMLIntakeVerifierV11(
            self._repository,
            principal_verifier,
        )

    def create_request(self) -> PennyLocalITunesXMLIntakeRequestV11:
        principal = self._principal_verifier.resolve_active()
        prefix = self._principal_verifier.verified_lineage_prefix_sha256(
            principal,
            require_current_tip=True,
    ...
    def capture_selected_file(
        ...
        if not isinstance(request, PennyLocalITunesXMLIntakeRequestV11):
            raise ITunesWindowsXMLIntakeInvalidInput(
                "governed 1.1 intake requires an exact 1.1 intake request"
            )
        try:
            validated = PennyLocalITunesXMLIntakeRequestV11.model_validate(
                request.model_dump(mode="json")
            ...
            evidence = PennyLocalITunesXMLFileSelectionEvidenceV11(
                ...
                intake_request_sha256=validated.canonical_sha256,
                principal_id=validated.principal_authority.principal_id,
                principal_authority_artifact_id=validated.principal_authority.artifact_id,
                principal_authority_sha256=validated.principal_authority.canonical_sha256,
                installation_id=validated.principal_authority.installation_id,
                principal_lineage_prefix_sha256=validated.principal_lineage_prefix_sha256,
                byte_length=len(payload),
                byte_sha256=byte_sha256,
                source_receipt=receipt,
                source_receipt_artifact_sha256=receipt_sha256,
            ...
            if isinstance(exc, ITunesWindowsXMLIntakeInvalidInput):
                raise
            raise ITunesWindowsXMLIntakeInvalidInput(
                "governed iTunes XML 1.1 selection failed closed"
            ) from exc
...
def _verify_file_selection_evidence_structure_v11(
    evidence: PennyLocalITunesXMLFileSelectionEvidenceV11,
    *,
    principal_verifier: LocalPrincipalAuthorityVerifier,
    require_current_tip: bool = False,
    ...
    if not isinstance(evidence, PennyLocalITunesXMLFileSelectionEvidenceV11):
        return False
    try:
        validated = PennyLocalITunesXMLFileSelectionEvidenceV11.model_validate(
            evidence.model_dump(mode="json")
        )
        prefix = principal_verifier.verified_lineage_prefix_sha256(
            validated.intake_request.principal_authority,
